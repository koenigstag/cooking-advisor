...
function tryFetchModuleSrcWithExt(fullUrl) {
    return fetch(fullUrl)
        ...
        .then(rs => {
            if (rs.status === 200) {
                return rs.text().then(code => ({fullUrl, code}));
            } else {
...
function tryFetchModuleSrcExtOrNot(url, jsx) {
    ...
    const explicitUrl = url.startsWith('http://') || url.startsWith('https://');
...
const WorkerManager = ({compilerOptions}) => {
    ...
    const parseInWorker = async ({url, fullUrl, code}) => {
        ...
        if (fromCache) {
            ...
            return withFreeWorker(worker => worker.parseTsModule({
                fullUrl, code, compilerOptions, importMap
    ...
    return {
        fetchModuleData: url => fetchModuleSrc(url)
            .then(({fullUrl, code}) => parseInWorker({url, fullUrl, code})),
    };
};
