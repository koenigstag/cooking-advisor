...
org.klesun.tsBrowser.ParseTsModule_sideEffects = ({
    fullUrl, code, compilerOptions, ts, addPathToUrl, importMap,
}) => {
    const extension = fullUrl.replace(/^.*\./, '');
    const sourceFile = ts.createSourceFile(
        'ts-browser-generated-file.' + extension, code, compilerOptions.target
    );
    let jsCodeImports = '';
    ...
    for (const statement of sourceFile.statements) {
        ...
        if (kindName === 'ImportDeclaration') {
            ...
            if (!importClause) {
                ...
                jsCodeImports += es6ToDestr(code, importClause) + ' = ' + assignedValue + ';\n';
            }
